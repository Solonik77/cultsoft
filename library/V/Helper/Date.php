...
$timestamp = (
$timestamp === false) ? getdate() : getdate(
$timestamp);
        if (
$timestamp['year'] < 1980) {
            return (1 << 21 | 1 << 16);
...
$timestamp['year'] -= 1980;
        // What voodoo is this? I have no idea... Geert can explain it though,       // and that's good enough for me.
        return (
$timestamp['year'] << 25 | 
$timestamp['mon'] << 21 | 
$timestamp['mday'] << 16 | 
...
$ timezone that to find the offset of
    * @param string 
$ |boolean  timezone used as the baseline
    * @return integer
    */
...
$local = (
$local === true) ? date_default_timezone_get() : (string) 
$local;
        // Cache key name
...
$time_there = new DateTime('now', 
$remote);
            
$time_here = new DateTime('now', 
$local);
            // Find the offset
...
$cache];
    ...
    *
    * @param integer 
$ amount to increment each step by, 1 to 30
    * @param integer 
$ start value
    * @param integer 
$ end value
...
$step = 5)
    {
        // Because there are the same number of minutes as seconds in this set,       // we choose to re-use seconds(), rather than creating an entirely new
...
$ number of month
    * @param integer 
$ number of year to check month, defaults to the current year
...
$start = (
$start === false) ? date('Y') - 5 : (int) 
$start;
        
$end = (
$end === false) ? date('Y') + 5 : (int) 
$end;
...
$output = preg_split('/[^a-z]+/',           strtolower((string) 
$output));
        // Invalid output
        if (empty(
$output))
...
$ formatting string
    * @return string
    */
    public static function timespan_string(
$time1, 
$time2 = null,       
$output = 'years,months,weeks,days,hours,minutes,seconds')
    {
        if (
$difference = V_Helper_Date::timespan(
$time1, 
$time2, 
$output) and is_array(
$difference)) {
...
$difference = 
$difference . ' ' . (
$difference === 1 ? inflector::singular(
$output) : 
$output);
        }
        return 
...
$date
    * @return boolean
    */
    public static function is_empty_date(
$date)
    {
