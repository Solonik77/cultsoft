...
$filename)
    {
        ...
        if (! (is_file(
$filename) and is_readable(
$filename)))
            return false;
        // Get the extension from the filename
        
$extension = strtolower(           substr(strrchr(
$filename, '.'), 1));
        if (preg_match('/^(?:jpe?g|png|[gt]if|bmp|swf)
$/',               
$extension)) {
...
$ file to be split
    * @param string 
$ directory to output to, defaults to the same directory as the file
    * @param integer 
$ size, in MB, for each chunk to be
...
$output_dir = (
$output_dir == false) ? pathinfo(           str_replace('\\', '/',               realpath(
$filename)), PATHINFO_DIRNAME) : str_replace('\\', '/', realpath(
$output_dir));
...
$piece_size = 1024 * 1024 * (int) 
$piece_size; // Size in bytes
...
$piece_name = 
$filename . '.' . str_pad(
$piece, 3, '0',               STR_PAD_LEFT);
            
$piece_open = @fopen(
$piece_name, 'wb+') or die('Could not write piece ' . 
$piece_name);
...
$chunk)) {
                fwrite(
$piece_open,                   
$data) or die('Could not write to open piece ' . 
$piece_name);
...
$ split filename, without .000 extension
    * @param string 
$ output filename, if different then an the filename
...
$output_file = @fopen(
$output, 'wb+') or die('Could not open output file ' . 
$output);
        // Read each piece
        while (
$piece_open = @fopen(               (
...
$path);
    ...
    */
    public static function isDirWriteable(
$dir)
    {
        if (is_dir(
$dir) && is_writable(
...
$file = 
$dir . DIRECTORY_SEPARATOR . uniqid(                   mt_rand()) . '.tmp';
