...
class Vendor_Helper_Html {
    ...
    /**
     * Perform a self::specialchars() with additional URL specific encoding.
     *
     * @param string $ string to convert
     * @param boolean $ encode existing entities
     * @return string
    ...
    {
        return str_replace(' ', '%20', self::specialchars($str, $double_encode));
    }
    ...
    public static function anchor($uri, $title = null, $attributes = null, $protocol = null)
    {
        ...
        return // Parsed URL
        '<a href="' . self::specialurlencode($site_url, false) . '"' . // Attributes empty? Use an empty string
        (is_array($attributes) ? self::attributes($attributes) : '') . '>' . // Title empty? Use the parsed URL
    ...
    public static function file_anchor($file, $title = null, $attributes = null, $protocol = null)
    {
        ...
        '<a href="' . self::specialurlencode(url::base(false, $protocol) . $file, false) . '"' . // Attributes empty? Use an empty string
        (is_array($attributes) ? self::attributes($attributes) : '') . '>' . // Title empty? Use the filename part of the URI
    ...
    public static function anchor_array(array $array)
    {
        ...
        foreach($array as $link => $title) {
            ...
            $anchors[] = self::anchor($link, $title);
        }
        return $anchors;
    }
    ...
    public static function mailto($email, $title = null, $attributes = null)
    {
        ...
        // Parse attributes
        empty($attributes) or $attributes = self::attributes($attributes);
    ...
    public static function breadcrumb($segments = null)
    {
        ...
        while ($segment = array_pop($segments)) {
            $array[] = self::anchor(// Complete URI for the URL
    ...
    {
        return self::link($style, 'stylesheet', 'text/css', $media, $index);
    }
    ...
    public static function link($href, $rel, $type, $media = false, $index = false)
    {
        ...
        if (is_array($href)) {
            foreach($href as $_href) {
                ...
                $compiled .= self::link($_href, $_rel, $_type, $_media, $index);
            }
        } else {
    ...
    public static function image($src = null, $alt = null, $index = false)
    {
        ...
        if (strpos($attributes['src'], '://') === false) {
            ...
            $attributes['src'] = Vendor_Helper_Url::base($index) . $attributes['src'];
        }
        return '<img' . self::attributes($attributes) . ' />';
    }

    public static function tplImage($src = null,  $alt = null)
    {
        $attributes = is_array($src) ? $src : array('src' => $src);
        $attributes['src'] = 'static/view_resources/' . App::config()->project->template . '/images/' . $attributes['src'];
        return self::image($attributes, $alt);
    }
    ...
    public static function attributes($attrs)
    {
        ...
        foreach($attrs as $key => $val) {
            $compiled .= ' ' . $key . '="' . self::specialchars($val) . '"';
        }
        return $compiled;
    }
}
