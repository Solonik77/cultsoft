 ...
 */
 public function process($image, $actions, $dir, $file, $render = false)
 {
  ...
  $quality = (int) V_helper_Arr::remove('quality', $actions);
...
if ($error = exec(escapeshellcmd($this->dir . 'convert' . $this->ext) . ' -quality ' . $quality . '% ' . $this->cmd_image . ' ' . $this->new_image)) {
 ...
 if ($render !== false) {
  ...
  switch (substr($file, strrpos($file, '.') + 1)) {case 'jpg':case 'jpeg': header('Content-Type: image/jpeg'); break;case 'gif': header('Content-Type: image/gif'); break;case 'png': header('Content-Type: image/png'); break;
  }
  echo $contents;
 }
}
