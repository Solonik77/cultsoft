...
$instance;
  ...
  empty(
$instance) and 
$instance = new App_Encrypt((array) 
$config);
 ...
 *
 * @param array 
$ |stringcustom configuration or config group name
...
$name . ' group is not defined in your configuration.');
  }
...
$config['iv_size'] = mcrypt_get_iv_size(
$config['cipher'], 
$config['mode']);
...
$data)
{
 ...
 if (App_Encrypt::$rand === null) {
  if (App::isWin()) {// Windows only supports the system random number generatorApp_Encrypt::$rand = MCRYPT_RAND;
  } else {if (defined('MCRYPT_DEV_URANDOM')) { // Use /dev/urandom App_Encrypt::$rand = MCRYPT_DEV_URANDOM;} elseif (defined('MCRYPT_DEV_RANDOM')) { // Use /dev/random App_Encrypt::$rand = MCRYPT_DEV_RANDOM;} else { // Use the system random number generator App_Encrypt::$rand = MCRYPT_RAND;}
  }
 }
...
$iv = mcrypt_create_iv(
$this->config['iv_size'], App_Encrypt::$rand);
