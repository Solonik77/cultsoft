...
class App_Image {
 ...
 public static 
$allowed_types = array
 ( IMAGETYPE_GIF => 'gif', IMAGETYPE_JPEG => 'jpg', IMAGETYPE_PNG => 'png', IMAGETYPE_TIFF_II => 'tiff', IMAGETYPE_TIFF_MM => 'tiff', );
 // Adapter instance
 protected 
$adapter;
 // Adapter actions
...
$check;
  ...
  (
$check === null) and 
$check = function_exists('getimagesize');
...
throw new App_Exception('The Image library requires the getimagesize() PHP function, which is not available in your installation.');
  ...
  if (! is_file(
$image))
throw new App_Exception('The specified image, ' . 
$image . ', was not found. Please verify that images exist by using file_exists() before manipulating them.');
...
$image_info[2]]))
throw new App_Exception('The specified image, ' . 
$image . ', is not an allowed image type.');
...
$image_info[0],  'height' => 
$image_info[1],  'type' => 
$image_info[2],  'ext' => App_Image::$allowed_types[
$image_info[2]],  'mime' => 
$image_info['mime']
);
  // Load configuration
...
$this->config = (array) 
$config + App::config()->image->toArray();
  // Set adapter class name
  
$adapter = 'App_Image_Adapter_' . ucfirst(
$this->config['adapter']);
  // Load the adapter
  if (! Zend_Loader::loadClass(
$adapter))
throw new App_Exception('The ' . 
$this->config['adapter'] . ' adapter for the Image  library could not be found');
...
$this->config['params']);
  ...
  if (! (
$this->adapter instanceof App_Image_Adapter))
throw new App_Exception('The ' . 
$this->config['adapter'] . ' adapter for the %s library must implement the ' . get_class(
$this) . ' interface');
 }
...
throw new App_Exception('The ' . 
$property . ' property does not exist in the ' . get_class(
$this) . ' class.');
  }
 }
 ...
 * wish to use height as master dim, set 
$image->master_dim = App_Image::HEIGHT
...
$ height
 * @param integer 
$ one of: App_Image::NONE, App_Image::AUTO, App_Image::WIDTH, App_Image::HEIGHT
 * @return object
 */
...
$master = null)
 {
  if (! 
$this->valid_size('width', 
$width))
throw new App_Exception('The width you specified, ' . 
$width . ', is not valid.');

  if (! 
$this->valid_size('height', 
$height))
throw new App_Exception('The height you specified, ' . 
$height . ', is not valid.');
...
$this->valid_size('top', 
$top))
throw new App_Exception('Invalid ' . 
$top . ' top');

  if (! 
$this->valid_size('left', 
$left))
throw new App_Exception('Invalid ' . 
$left . ' left');
...
$direction)
 {
  if (
$direction !== App_Image::HORIZONTAL AND 
$direction !== App_Image::VERTICAL)
...
$this->actions['quality'] = max(1, min(
$amount, 100));
...
$this->actions['sharpen'] = max(1, min(
$amount, 100));
...
$dir = pathinfo(
$new_image, PATHINFO_DIRNAME);
  
$file = pathinfo(
$new_image, PATHINFO_BASENAME);
  // Normalize the path
...
$dir)) . '/';
  ...
  if (! is_writable(
$dir))
throw new App_Exception('The specified directory, ' . 
$dir . ', is not writable.');
...
$value)) {
  if (! in_array(
$value, array('top', 'bottom', 'center')))return false;
 } else {
...
$value)) {
  if (! in_array(
$value, array('left', 'right', 'center')))return false;
 } else {
...
case 'master':
 if (
$value !== App_Image::NONE AND
  
$value !== App_Image::AUTO AND
  
$value !== App_Image::WIDTH AND
  
$value !== App_Image::HEIGHT)
  return false;
 break;
  }
