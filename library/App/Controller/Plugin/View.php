...
$this->_initZendLayout(
$request);

$this->_loadDefaultTemplateResources();
if (
$this->_isBackofficeController) {
...
$this->_viewRenderer->setView(
$this->_view)->setViewBasePathSpec(
$this->_templatePath)->setViewScriptPathSpec(  ((
...
$this->_isBackofficeController !== true) ? 'html/:module/:action.:suffix' : 'html/:action.:suffix'))->setViewSuffix('phtml');
 ...
 private function _initZendLayout(Zend_Controller_Request_Abstract 
$request)
 {
  
$layout = Zend_Layout::startMvc(  array('layoutPath' => 
$this->_templatePath . 'layouts/' , 'layout' => 'default' , 'mvcSuccessfulActionOnly' => ('development' === APPLICATION_ENV)));
  if (! 
$this->_isBackofficeController) {
if (
$request->getModuleName() == 'default' and 
$request->getControllerName() == 'index' and 
$request->getActionName() == 'index' and file_exists(
$this->_templatePath . 'layouts/homepage.phtml')) {
 
$layout->setLayout('homepage');
} else
if (file_exists(
$this->_templatePath . 'layouts/' . 
...
$this->_view->getHelper('DeclareVars')->declareVars(array('uploadedIMG' => App::baseUri() . 'static/upload/images/' ,'requestLang' => 
$requestLang , 'projectTitle' => App::config()->project->title->
$requestLang , 'baseUrl' => App::baseUri() ,'tplJS' => App::baseUri() . ((! 
$this->_isBackofficeController) ? 'static/view_resources/' . App::config()->project->template . '/clientscripts/' : 'static/system/admin/clientscripts/') ,'tplCSS' => App::baseUri() . ((! 
$this->_isBackofficeController) ? 'static/view_resources/' . App::config()->project->template . '/css/' : 'static/system/admin/css/') ,'tplIMG' => App::baseUri() . ((! 
$this->_isBackofficeController) ? 'static/view_resources/' . App::config()->project->template . '/images/' : 'static/system/admin/images/')));
 }
}
