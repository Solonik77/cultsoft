...
class V_Helper_Text_Typo extends V_Helper_Text
{
     ...
     */
    protected function _cfgSetTagsFlag($tags, $flag, $value, 
    ...
    {
        $this->_cfgSetTagsFlag($tags, 
        self::TR_TAG_ALLOWED, true);
    }
    ...
    {
        $this->_cfgSetTagsFlag($tags, 
        self::TR_TAG_PREFORMATTED, true, false);
    }
    ...
    {
        $this->_cfgSetTagsFlag($tags, 
        self::TR_TAG_NO_TYPOGRAPHY, true, false);
    }
    ...
    {
        $this->_cfgSetTagsFlag($tags, 
        self::TR_TAG_IS_EMPTY, true, false);
    }
     ...
     */
    function cfgSetTagChilds($tag, $childs, 
    $isContainerOnly = false, $isChildOnly = false)
    {
        if(! isset($this->tagsRules[$tag]))
            throw new Exception(
            "Тег $tag отсутствует в списке разрешённых тегов");
        ...
        foreach($childs as $child)
        {
            ...
            if(! isset(
            $this->tagsRules[$child]))
                throw new Exception(
                "Тег $child отсутствует в списке разрешённых тегов");
            if(! isset(
    ...
    function cfgSetTagParamsAutoAdd($tag, $params)
    {
        if(! isset($this->tagsRules[$tag]))
            throw new Exception(
            "Tag $tag is missing in allowed tags list");
        if(! is_array($params))
            $params = array($params);
        if(! isset(
    ...
    protected function goToPosition($position)
    {
        ...
        if($this->curPos < $this->textLen)
        {
            ...
            $this->curCh);
            $this->curChClass = $this->getCharClass(
            $this->curChOrd);
        }
        else
        {
     ...
     */
    protected function matchChClass($chClass, 
    $skipSpaces = false)
    {
    ...
    protected function tag(&$tag, &$params, &$content, &$short)
    {
        ...
        if(! empty(
        $this->tagsRules[$tag][self::TR_TAG_PREFORMATTED]))
        {
            $this->state = self::STATE_INSIDE_PREFORMATTED_TAG;
        }
        elseif(! empty(
        $this->tagsRules[$tag][self::TR_TAG_CONTAINER]))
        {
            $this->state = self::STATE_INSIDE_NOTEXT_TAG;
        }
        elseif(! empty(
        $this->tagsRules[$tag][self::TR_TAG_NO_TYPOGRAPHY]))
        {
        ...
        {
            $this->eror(
            "Неверный закрывающийся тег $closeTag. Ожидалось закрытие $tag");
            // $this->restoreState();
        }
    ...
    protected function preformatted(&$content = '', 
    $insideTag = null)
    {
        while($this->curChClass)
        {
            ...
            $content .= isset(
            $this->entities2[$this->curCh]) ? $this->entities2[$this->curCh] : $this->curCh;
            $this->getCh();
        }
    }
    ...
    protected function tagOpen(&$name, &$params, &$short = false)
    {
        ...
        if($this->curCh != '>' && $this->curCh != '/')
            ...
            // Короткая запись тега
        $short = ! empty(
        $this->tagsRules[$name][self::TR_TAG_SHORT]);
    ...
    protected function tagParamValue(&$value, $quote)
    {
        if($quote !== false)
        {
            ...
            while($this->curChClass && ($this->curCh != $quote || $escape))
            {
                ...
                $value .= isset(
                $this->entities1[$this->curCh]) ? $this->entities1[$this->curCh] : $this->curCh;
    ...
    protected function makeTag($tag, $params, $content, $short, 
    $parentTag = null)
    {
        $tag = strtolower($tag);
        // Получаем правила фильтрации тега
        $tagRules = isset(
        $this->tagsRules[$tag]) ? $this->tagsRules[$tag] : null;
        // Проверка - родительский тег - контейнер, содержащий только другие теги (ul, table, etc)
        $parentTagIsContainer = $parentTag && isset(
        $this->tagsRules[$parentTag][self::TR_TAG_CONTAINER]);
        // Вырезать тег вместе с содержанием
        if($tagRules && isset(
        $this->tagsRules[$tag][self::TR_TAG_CUT]))
            return '';
            // Позволен ли тег
        if(! $tagRules || empty(
        ...
        {
            if(! isset(
            $this->tagsRules[$parentTag][self::TR_TAG_CHILD_TAGS][$tag]))
                return '';
        }
        ...
        {
            if(! isset(
            $tagRules[self::TR_TAG_PARENT][$parentTag]))
                return $content;
        }
        $resParams = array();
        foreach($params as $param => $value)
        {
            ...
            $tagRules[self::TR_PARAM_ALLOWED][$param]) ? $tagRules[self::TR_PARAM_ALLOWED][$param] : false;
            if(empty(
            $paramAllowedValues))
                continue;
            ...
            {
                $this->eror(
                "Недопустимое значение для атрибута тега $tag $param=$value");
                continue;
                // Если атрибут тега помечен как разрешённый, но правила не указаны - смотрим в массив стандартных правил для атрибутов
            }
            elseif($paramAllowedValues === true && ! empty(
            $this->defaultTagParamRules[$param]))
            {
            ...
            $paramAllowedValues))
            {
                switch ($paramAllowedValues)
                {
                    case '#int':
                        ...
                        {
                            $this->eror(
                            "Недопустимое значение для атрибута тега $tag $param=$value. Ожидалось число");
                            continue (2);
                        }
                        break;
                    case '#text':
                    ...
                    case '#link':
                        ...
                        {
                            $this->eror(
                            'Попытка вставить JavaScript в URI');
                            continue (2);
                        }
                        ...
                        {
                            $this->eror(
                            'URI: Первый символ адреса должен быть буквой или цифрой');
                            continue (2);
                        }
                    ...
                    case '#image':
                        ...
                        {
                            $this->eror(
                            'Попытка вставить JavaScript в пути к изображению');
                            continue (2);
                        }
                        // HTTP в начале если нет
                        if(! preg_match(
                    ...
                    default:
                        $this->eror(
                        "Неверное описание атрибута тега в настройке Jevix: $param => $paramAllowedValues");
                        continue (2);
                        break;
                }
            }
        ...
        if(! isset(
        $tagRules[self::TR_TAG_IS_EMPTY]) or ! $tagRules[self::TR_TAG_IS_EMPTY])
        {
            if(! $short && empty(
            $content))
                return '';
    ...
    protected function anyThing(&$content = '', $parentTag = null)
    {
        ...
        while($this->curChClass)
        {
            ...
            // <Тег> кекст </Тег>
            if($this->curCh == '<' && $this->tag(
            $tag, $params, $text, 
            $shortTag))
            {
                // Преобразуем тег в текст
                $tagText = $this->makeTag(
                $tag, 
                $params, 
                $text, 
                $shortTag, 
            ...
            elseif($this->curCh == '<')
            {
                ...
                $name))
                {
                    ...
                    {
                        $this->eror(
                        'Не ожидалось закрывающегося тега ' . $name);
                    }
                }
                else
                {
    ...
    protected function text(&$text)
    {
        ...
        while(($this->curCh != '<') && $this->curChClass)
        {
            ...
            if(! $spCount && $this->curCh == '&' && $this->htmlEntity(
            $entity))
            {
                $text .= isset(
                $this->entities2[$entity]) ? $this->entities2[$entity] : $entity;
            }
            elseif($typoEnabled && ($this->curChClass & self::PUNCTUATUON) && $this->punctuation(
            $punctuation))
            {
            ...
            elseif($typoEnabled && ($spCount || $newLine) && $this->curCh == '-' && $this->dash(
            $dash))
            {
                // Тире
                $text .= $dash;
            ...
            elseif($typoEnabled && ($this->curChClass & self::HTML_QUOTE) && $this->quote(
            $spCount, $quote, 
            $closed))
            {
                // Кавычки
                ...
                $closed, 
                $closed ? $this->quotesOpened : $this->quotesOpened - 1);
                if($spCount)
                    $quote = ' ' . $quote;
                $text .= $quote;
                $newWord = true;
            ...
            elseif($this->isAutoBrMode && $this->skipNL(
            $brCount))
            {
                // Перенос строки
                $br = $this->br . $this->nl;
            ...
            elseif($newWord && $this->isAutoLinkMode && ($this->curChClass & self::LAT) && $this->openedTag != 'a' && $this->url(
            $url, $href))
            {
                // URL
                $text .= $this->makeTag(
    ...
    {
        $str = '';
        $strEnd = min($this->curPos + 8, 
        $this->textLen);
        for($i = $this->curPos; $i < $strEnd; $i ++)
        {
            $str .= $this->textBuf[$i];
        }
        $this->errors[] = array('message' => $message , 'pos' => $this->curPos , 'ch' => $this->curCh , 'line' => 0 , 'str' => $str);
    }
}
