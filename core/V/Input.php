...
class V_Input
{
        ...
        if(V_Input::$instance === null)
        {
            ...
            'register_globals'))
            {
                ...
                $_REQUEST['GLOBALS']))
                {
                    ...
                    exit(
                    'Global variable overload attack.');
                }
            ...
            if(is_array($_GET))
            {
                ...
                {
                    // Sanitize $_GET
                    $_GET[$this->clean_input_keys(
                    $key)] = $this->clean_input_data(
                    $val);
                }
            }
            else
            ...
            App::Log(
            'Global GET, POST and COOKIE data sanitized', 
            Zend_Log::DEBUG);
        }
    }
    ...
    public function xss_clean($data, $tool = null)
    {
        ...
        {
            App::Log(
            'Unable to use Input::xss_filter_' . $tool . '(), no such method exists', 
            Zend_Log::ERR);
            $tool = 'default';
        }
    ...
    public function clean_input_data($str)
    {
        if(is_array($str))
        {
            ...
            {
                // Recursion!
                $new_array[$this->clean_input_keys(
                $key)] = $this->clean_input_data(
                $val);
            }
            return $new_array;
