...
class App_Cache
{
    ...
    public static function getInstance($instanceId = null)
    {
        ...
        if($instanceId == null)
        {
            ...
            $instanceId))
            {
                $instanceId = 'getInstance' . ucfirst(
                strtolower(
                $instanceId));
    ...
    public function __construct()
    {
        if(App_Cache::$instance === null)
        {
            ...
            $this->initInstance(
            array('id' => 'System' , $systemCacheFrontend));
    ...
    {
        $return = null;
        if((strlen($method) > 11) and (substr(
        $method, 0, 11) == 'getInstance'))
        {
            $id = ucfirst(
            strtolower(
            substr($method, 11)));
    ...
    {
        $data = null;
        if(! ($data = $this->cache->System->load(
        'AclRoles')))
        {
     ...
     */
    public function initInstance(array $instanceId, 
    $frontendOptions = null, $backendOptions = null)
    {
        $frontendOptions = (! is_null(
        $frontendOptions) and is_array(
        $frontendOptions) and ! empty(
        $frontendOptions)) ? $frontendOptions : $this->_defaultFrontendOptions;
        $backendOptions = (! is_null(
        $backendOptions) and is_array(
        $backendOptions) and ! empty(
        $backendOptions)) ? $backendOptions : $this->_defaultBackendOptions;
        if(is_array($instanceId) and isset(
        $instanceId['id']))
        {
            $instanceId['id'] = ucfirst(
            strtolower(
            $instanceId['id']));
            ...
            {
                $backend = (isset(
                $instanceId['backend'])) ? ucfirst(
                strtolower(
                $instanceId['backend'])) : 'File';
                $this->cache->$instanceId['id'] = Zend_Cache::factory(
                'Core', 
                $backend, 
                $frontendOptions, 
                $backendOptions);
        ...
        else
        {
            ...
            'Cache instance ID must be valids array');
        }
