    ...
    */
    protected function _initConfiguration()
    {
        ...
        if (APPLICATION_ENV == 'development' and file_exists(
            ...
            $options->merge(new Zend_Config_Ini(
                    VAR_PATH . 'configuration_development.ini',
                    null));
        }
        if (file_exists(
    ...
    */
    private function _setLanguage()
    {
        ...
        App::front()->getRouter()->addRoute('default_multilingual',
            new Zend_Controller_Router_Route(':requestLang/:module/:controller/:action/*',
                array('module' => 'default' , 'controller' => 'index' , 'action' => 'index' , 'requestLang' => $system_lang),
    ...
    */
    private function _initDatabase()
    {
        try {
            $config = App::config();
            App::setDb(
                Zend_Db::factory($config->database->adapter,
    ...
    */
    public function run()
    {
        try {
            ...
            $front->setParam('bootstrap', $this)->setParam('prefixDefaultModule',
                true);
            $front->returnResponse(
                true);
            $response = App::front()->dispatch();
            $response->setHeader('Expires',
                'Sat, 13 Apr 1985 00:30:00 GMT')->setHeader('Last-Modified',
                gmdate('D, d M Y H:i:s') . ' GMT')->setHeader('Cache-Control',
                'no-cache, must-revalidate')->setHeader('Cache-Control',
                'post-check=0,pre-check=0')->setHeader('Cache-Control',
                'max-age=0')->setHeader('Pragma', 'no-cache')->setHeader('Content-type',
            ...
            if ($level = 9 and ini_get('output_handler') !== 'ob_gzhandler' and (int) ini_get('zlib.output_compression') === 0) {
                ...
                if (stripos(@$_SERVER['HTTP_ACCEPT_ENCODING'],
                        'gzip') !== false) {
                    $compress = 'gzip';
                } elseif (stripos(@$_SERVER['HTTP_ACCEPT_ENCODING'],
                        'deflate') !== false) {
                    $compress = 'deflate';
                }
            }
            if (isset($compress) and $level > 0) {
                switch ($compress) {
                    case 'gzip':
                        ...
                        $response->setBody(
                            gzencode($response->getBody(),
                                $level));
                        break;
                    case 'deflate':
                        // Compress output using zlib (HTTP deflate)
                        $response->setBody(
                            gzdeflate($response->getBody(),
                                $level));
                        break;
                }
