...
class Bootstrap extends Zend_Application_Bootstrap_Bootstrap
{
    ...
    public function __construct($application)
    {
        ...
        array('basePath' => APPLICATION_PATH . 'resources/Site' , 'namespace' => 'Site'));
        $resourceLoader->addResourceTypes(
        array('model' => array('namespace' => 'Model' , 'path' => 'models') , 'dbtable' => array('namespace' => 'Model_DbTable' , 'path' => 'models/DbTable') , 'form' => array('namespace' => 'Form' , 'path' => 'forms') , 'model' => array('namespace' => 'Model' , 'path' => 'models') , 'plugin' => array('namespace' => 'Plugin' , 'path' => 'plugins') , 'service' => array('namespace' => 'Service' , 'path' => 'services') , 'helper' => array('namespace' => 'Helper' , 'path' => 'helpers') , 'viewhelper' => array('namespace' => 'View_Helper' , 'path' => 'views/helpers') , 'viewfilter' => array('namespace' => 'View_Filter' , 'path' => 'views/filters')));
        $resourceLoader = new Zend_Loader_Autoloader_Resource(
        array('basePath' => APPLICATION_PATH . 'resources/Admin' , 'namespace' => 'Admin'));
        $resourceLoader->addResourceTypes(
        array('model' => array('namespace' => 'Model' , 'path' => 'models') , 'dbtable' => array('namespace' => 'Model_DbTable' , 'path' => 'models/DbTable') , 'form' => array('namespace' => 'Form' , 'path' => 'forms') , 'model' => array('namespace' => 'Model' , 'path' => 'models') , 'plugin' => array('namespace' => 'Plugin' , 'path' => 'plugins') , 'service' => array('namespace' => 'Service' , 'path' => 'services') , 'helper' => array('namespace' => 'Helper' , 'path' => 'helpers') , 'viewhelper' => array('namespace' => 'View_Helper' , 'path' => 'views/helpers') , 'viewfilter' => array('namespace' => 'View_Filter' , 'path' => 'views/filters')));
    ...
    {
        $options = new Zend_Config_Ini(
        VAR_PATH . 'configuration.ini', null, 
        true);
        if(APPLICATION_ENV == 'development' and file_exists(
        VAR_PATH . 'configuration_development.ini'))
        {
            $options->merge(
            new Zend_Config_Ini(
            VAR_PATH . 'configuration_development.ini', 
        ...
        if(file_exists(
        VAR_PATH . 'cache/configs/settings.ini'))
        {
            $options->merge(
            new Zend_Config_Ini(
            VAR_PATH . 'cache/configs/settings.ini', 
    ...
    private function _initErrorHandler()
    {
        ...
        App::Config()->syspath->log . "/system_log_" . date(
        'Y-m-d') . '.log'));
        App::setLog($logger);
    }
    ...
    private function _setLanguage()
    {
        ...
        App::front()->getRouter()->addRoute(
        'default', 
        new Zend_Controller_Router_Route(
        ':module/:controller/:action/*', 
        array('module' => 'default' , 'controller' => 'index' , 'action' => 'index' , 'requestLang' => $system_lang)));
        // add multilingual route
        App::front()->getRouter()->addRoute(
        'default_multilingual', 
        new Zend_Controller_Router_Route(
        ':requestLang/:module/:controller/:action/*', 
        array('module' => 'default' , 'controller' => 'index' , 'action' => 'index' , 'requestLang' => $system_lang), 
        array('requestLang' => '\w{2}')));
        App::front()->registerPlugin(
        new App_Controller_Plugin_Language());
    }
    ...
    private function _initSession()
    {
        ...
        new App_Session_SaveHandler_DbTable(
    ...
    {
        App::front()->registerPlugin(
        new App_Controller_Plugin_View());
    }
    ...
    private function _initAccess()
    {
        ...
        App::front()->registerPlugin(
        new App_Controller_Plugin_Access());
    }
    ...
    {
        App_Mail::setDefaultTransport(
        App::config()->mail->toArray());
    }
    ...
    private function _initDebug()
    {
        ...
        {
            App::front()->registerPlugin(
            new ZFDebug_Controller_Plugin_Debug(
            array('plugins' => array('Variables' , 'Html' , 'Database' => array('adapter' => array('standard' => App::DB())) , 'File' => array('basePath' => APPLICATION_PATH) , 'Memory' , 'Time' , 'Registry' , 'Cache' => array('backend' => App_Cache::getInstance(
            'File')->getBackend()) , 'Exception'))));
        }
    }
    ...
    public function run()
    {
        try
        {
            $front = $this->getResource(
            'FrontController');
            $front->setModuleControllerDirectoryName(
            '');
            $front->addModuleDirectory(
            APPLICATION_PATH . 'controllers' . DIRECTORY_SEPARATOR);
            ...
            $response->setHeader(
            'Expires', 
            'Sat, 13 Apr 1985 00:30:00 GMT')->setHeader(
            'Last-Modified', 
            gmdate(
            'D, d M Y H:i:s') . ' GMT')->setHeader(
            'Cache-Control', 
            'no-cache, must-revalidate')->setHeader(
            'Cache-Control', 
            'post-check=0,pre-check=0')->setHeader(
            'Cache-Control', 
            'max-age=0')->setHeader(
            'Pragma', 'no-cache')->setHeader(
            'Content-type', 
            'text/html; charset=' . App::config()->locale->charset);
            if($level = 9 and ini_get(
            'output_handler') !== 'ob_gzhandler' and (int) ini_get(
