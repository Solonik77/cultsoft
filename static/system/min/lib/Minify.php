...
class Minify
{
    ...
    public static function serve ($controller, $options = array())
    {
        ...
        $cgOptions = array('lastModifiedTime' => self::$_options['lastModifiedTime'] , 'isPublic' => self::$_options['isPublic']);
        ...
        if (null !== self::$_cache) {
            ...
            $encodingExtension = self::$_options['encodeMethod'] ? ('deflate' === self::$_options['encodeMethod'] ? '.zd' : '.zg') : '';
        ...
        if (! $cacheIsReady && self::$_options['encodeMethod']) {
            ...
            $content = ('deflate' === self::$_options['encodeMethod']) ? gzdeflate($content, self::$_options['encodeLevel']) : gzencode($content, self::$_options['encodeLevel']);
        }
        // add headers
    ...
    public static function setDocRoot ($unsetPathInfo = false)
    {
        if (isset($_SERVER['SERVER_SOFTWARE']) && 0 === strpos($_SERVER['SERVER_SOFTWARE'], 'Microsoft-IIS/')) {
            $_SERVER['DOCUMENT_ROOT'] = substr($_SERVER['PATH_TRANSLATED'], 0, strlen($_SERVER['PATH_TRANSLATED']) - strlen($_SERVER['SCRIPT_NAME']));
            if ($unsetPathInfo) {
    ...
    {
        return md5(serialize(array(Minify_Source::getDigest(self::$_controller->sources) , self::$_options['minifiers'] , self::$_options['minifierOptions'] , self::$_options['postprocessor'] , self::$_options['bubbleCssImports'])));
    }
