	...
	*/
	...
	function parse($text, &$errors)
	{
		...
		if (! empty($this->autoReplace)) {
			$this->text = str_ireplace($this->autoReplace['from'],
			$this->autoReplace['to'],
			$this->text);
		}
	...
	protected function restoreState($index = null)
	{
		...
		if ($index == null) {
			...
			$this->states = array_slice($this->states, 0,
			$index);
		}
	...
	$parentTag = null)
	{
		...
		foreach($params as $param => $value) {
			...
			if (is_array($paramAllowedValues) && ! in_array($value,
			$paramAllowedValues)) {
	...
	protected function anyThing(&$content = '', $parentTag = null)
	{
		...
		while ($this->curChClass) {
			...
			// <Тег> кекст </Тег>
			if ($this->curCh == '<' && $this->tag($tag, $params, $text,
			$shortTag)) {
				// Преобразуем тег в текст
				$tagText = $this->makeTag($tag,
				$params,
				$text,
				$shortTag,
				$parentTag);
	...
	protected function htmlEntity(&$entityCh)
	{
		...
		if ($this->matchCh('#')) {
			...
			$entityCh = html_entity_decode("&#$entityCode;",
			ENT_COMPAT, 'UTF-8');
			return true;
		} else {
			$entityName = '';
			...
			$entityCh = html_entity_decode("&$entityName;",
			ENT_COMPAT, 'UTF-8');
			return true;
		}
	}
