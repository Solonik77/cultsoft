...
$this->ext = (PHP_SHLIB_SUFFIX === 'dll') ? '.exe' : '';
  ...
  if (! is_file(realpath(
$config['directory']) . '/convert' . 
$this->ext))
throw new App_Exception('The ImageMagick directory specified does not contain a required program, convert' . 
$this->ext);
...
$error = exec(escapeshellcmd(
$this->dir . 'convert' . 
$this->ext) . ' -rotate ' . escapeshellarg(
