...
$config['directory']) . '/gm' . 
$this->ext))
throw new App_Exception('The GraphicsMagick directory specified does not contain a required program, gm.' . 
$this->ext);
  // Set the installation directory
  
$this->dir = str_replace('\\', '/', realpath(
$config['directory'])) . '/';
 }
...
$quality = (int) V_helper_Arr::remove('quality', 
$actions);
...
if (
$error = exec(escapeshellcmd(
$this->dir . 'gm' . 
$this->ext . ' convert') . ' -quality ' . 
$quality . '% ' . 
$this->cmd_image . ' ' . 
$this->new_image)) {
...
$this->tmp_image);
  switch (substr(
$file, strrpos(
$file, '.') + 1)) {case 'jpg':case 'jpeg': header('Content-Type: image/jpeg'); break;case 'gif': header('Content-Type: image/gif'); break;case 'png': header('Content-Type: image/png'); break;
  }
  echo 
$contents;
 }
...
$dir = (
$dir === App_Image::HORIZONTAL) ? '-flop' : '-flip';
...
$error = exec(escapeshellcmd(
$this->dir . 'gm' . 
$this->ext . ' convert') . ' -rotate ' . escapeshellarg(
$amt) . ' -background transparent ' . 
$this->cmd_image . ' ' . 
$this->cmd_image)) {
...
return false;
 ...
 {
  return array_slice(getimagesize(
$this->tmp_image), 0, 2, false);
 }
