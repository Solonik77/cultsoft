...
$create = 'imagecreatefrompng';
 break;
  ...
  switch (strtolower(substr(strrchr(
$file, '.'), 1))) {
case 'jpg':
case 'jpeg':
...
$quality = V_helper_Arr::remove('quality', 
$actions);
...
$file);
 ...
 switch (
$save) {
  case 'imagejpeg':header('Content-Type: image/jpeg');break;
  case 'imagegif':header('Content-Type: image/gif');break;
  case 'imagepng':header('Content-Type: image/png');break;
 }
...
$flipped = 
$this->imagecreatetransparent(
$width, 
$height);

  if (
$direction === App_Image::HORIZONTAL) {
for (
$x = 0; 
$x < 
$width; 
...
$properties['height'] = round(
$height * (substr(
$properties['height'], 0, -1) / 100));
  }
...
$width);
  ...
  if (
$properties['master'] === App_Image::AUTO) {
...
$properties['width']) > (
$height / 
$properties['height'])) ? App_Image::WIDTH : App_Image::HEIGHT;
  }

  if (empty(
$properties['height']) OR 
$properties['master'] === App_Image::WIDTH) {
...
$width);
  ...
  if (empty(
$properties['width']) OR 
$properties['master'] === App_Image::HEIGHT) {
...
$max_reduction_width = round(
$properties['width'] * 1.1);

$max_reduction_height = round(
$properties['height'] * 1.1);
// Reduce the size using an O(2n) algorithm, until it reaches the maximum reduction
while (
$pre_width / 2 > 
$max_reduction_width AND 
$pre_height / 2 > 
$max_reduction_height) {
...
if (
$status = imagecopyresized(
$img, 
$this->tmp_image, 0, 0, 0, 0, 
$pre_width, 
$pre_height, 
$width, 
$height)) {
...
$height);
  ...
  imagecopyresized(
$img, self::$blank_png, 0, 0, 0, 0, 
$width, 
$height, self::$blank_png_width, self::$blank_png_height);
  // Prevent the alpha from being lost
  imagealphablending(
$img, false);
  imagesavealpha(
