...
if (! empty($this->file)) {
 ...
 while ($read_line = fgets($file_source)) {
  if ($file_line >= $start_line) {if ($file_line === $line) { // Wrap the text of this line in <span> tags, for highlighting 
  $read_line = '<span>' . v_helper_html::specialchars($read_line) . '</span>';} else { $read_line = v_helper_html::specialchars($read_line);}$source .= $read_line;
  }
  if (++ $file_line > $end_line) {
  // Stop reading lines
  fclose($file_source);break;
  }
 }
 $layout->type = $type;
 $layout->file = $file;
...
if (isset($trace['function'])) {
 ...
 if (isset($trace['class'])) {
  $line[] = $trace['class'];
  if (isset($trace['type'])) {$line[] .= $trace['type'];
  }
 }
 $line[] = $trace['function'] . ($inline ? ' ' : '(');
 $args = array();
 if (! empty($trace['args'])) {
  foreach($trace['args'] as $arg) {if (is_string($arg) and file_exists($arg)) { // Sanitize path
   $arg = $this->debug_path($arg);}$args[] = '<code>' . V_Helper_Text::limit_chars( v_helper_html::specialchars(  self::debug_var($arg)), 50, '...') . '</code>';
  }
 }
...
case 'object':
 ...
 if ($recursion === true and in_array($hash,$objects)) {
  ...
  $objects[] = $hash;
  foreach($object->getProperties() as $property) {$out .= ($more === true ? ', ' : '') . $property->getName() . ' => ';if ($property->isPublic()) { $out .= self::debug_var($property->getValue($var),  true);} elseif ($property->isPrivate()) { $out .= '*PRIVATE*';} else { $out .= '*PROTECTED*';}$more = true;
  }
 }
 return $out . ' }';
case 'array':
 ...
 foreach((array) $var as $key => $val) {
  if (! is_int($key)) {$key = self::debug_var($key, true) . ' => ';
  } else {$key = '';
  }
