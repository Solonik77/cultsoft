...
throw new App_Exception('The specified image, ' . $image . ', is not an allowed image type.');
  ...
  $this->image = array
  ('file' => str_replace('\\', '/', realpath($image)),  'width' => $image_info[0],  'height' => $image_info[1],  'type' => $image_info[2],  'ext' => App_Image::$allowed_types[$image_info[2]],  'mime' => $image_info['mime']
);
  // Load configuration
...
throw new App_Exception('The master dimension specified is not valid.');
  ...
  ('width' => $width,  'height' => $height,  'master' => $master,  );
