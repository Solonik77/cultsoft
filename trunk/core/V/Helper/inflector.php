...
class V_Helper_Inflector
{
    ...
    public static function uncountable($str)
    {
        ...
        {
            // Cache uncountables
            V_Helper_Inflector::$uncountable = Kohana::config(
            'inflector.uncountable');
            // Make uncountables mirroed
            V_Helper_Inflector::$uncountable = array_combine(
            V_Helper_Inflector::$uncountable, 
            V_Helper_Inflector::$uncountable);
        }
        return isset(
        V_Helper_Inflector::$uncountable[strtolower(
        $str)]);
    }
    ...
    public static function singular($str, $count = null)
    {
        ...
        if(empty(V_Helper_Inflector::$irregular))
        {
            ...
            V_Helper_Inflector::$irregular = Kohana::config(
            'inflector.irregular');
        }
        if($irregular = array_search($str, 
        V_Helper_Inflector::$irregular))
        {
            $str = $irregular;
        }
        elseif(preg_match('/[sxz]es$/', $str) or preg_match(
        '/[^aeioudgkprt]hes$/', $str))
        {
            // Remove "es"
            $str = substr(
        ...
        elseif(substr($str, - 1) === 's' and substr(
        $str, - 2) !== 'ss')
        {
            $str = substr($str, 
            0, - 1);
    ...
    public static function plural($str, $count = null)
    {
        ...
        if(isset(
        V_Helper_Inflector::$irregular[$str]))
        {
            $str = V_Helper_Inflector::$irregular[$str];
        }
        elseif(preg_match('/[sxz]$/', $str) or preg_match(
        '/[^aeioudgkprt]h$/', $str))
        {
            $str .= 'es';
        }
