...
class V_Helper_Upload
{
    ...
    $directory = null, $chmod = 0644)
    {
        ...
        $directory = rtrim(
        $directory, '/') . '/';
        if(! is_dir($directory) and Kohana::config(
        'upload.create_directories') === true)
        {
        ...
        if(! is_writable($directory))
            ...
            'The upload destination folder, ' . $directory . ', does not appear to be writable.');
        if(is_uploaded_file($file['tmp_name']) and move_uploaded_file(
        $file['tmp_name'], 
    ...
    {
        return (is_array($file) and isset(
        $file['error']) and isset($file['name']) and isset(
        $file['type']) and isset(
        $file['tmp_name']) and isset(
        $file['size']));
    }
    ...
    {
        return (isset($file['tmp_name']) and isset(
        $file['error']) and is_uploaded_file(
        $file['tmp_name']) and (int) $file['error'] === UPLOAD_ERR_OK);
    }
