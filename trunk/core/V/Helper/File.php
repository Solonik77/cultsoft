    ...
    public static function mime($filename)
    {
        ...
        if (! app::is_win()) {
            ...
            if ($command = trim(
                ...
                return trim(
                    exec($command . ' -bi ' . escapeshellarg($filename)));
            }
        }
    ...
    */
        ...
        $piece_size = 10)
    {
        ...
        $output_dir = ($output_dir == false) ? pathinfo(
            str_replace('\\', '/',
                realpath($filename)), PATHINFO_DIRNAME) : str_replace('\\', '/', realpath($output_dir));
        ...
        while (! feof($input_file)) {
            // Open a new piece
            $piece_name = $filename . '.' . str_pad($piece, 3, '0',
                STR_PAD_LEFT);
            ...
            while ($read < $piece_size and $data = fread($input_file, $chunk)) {
                fwrite($piece_open,
                    $data) or die('Could not write to open piece ' . $piece_name);
                $read += $chunk;
            }
    ...
    */
    public static function join($filename, $output = false)
    {
        ...
        while ($piece_open = @fopen(
                ($piece_name = $filename . '.' . str_pad($piece, 3, '0', STR_PAD_LEFT)), 'rb')) {
