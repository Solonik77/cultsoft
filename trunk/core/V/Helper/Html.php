...
class V_Helper_Html
{
     ...
     * @return string
     */
    public static function specialurlencode($str, 
    $double_encode = true)
    {
        return str_replace(' ', '%20', 
        V_Helper_Html::specialchars($str, 
        $double_encode));
    }
     ...
     * @return string
     */
    public static function anchor($uri, $title = null, 
    $attributes = null, $protocol = null)
    {
        if($uri === '')
        {
            $site_url = url::base(
        ...
        {
            if(V_Helper_Html::$windowed_urls === true and empty(
            $attributes['target']))
            {
        ...
        return // Parsed URL
'<a href="' . V_Helper_Html::specialurlencode(
        $site_url, false) . '"' . // Attributes empty? Use an empty string
(is_array(
        $attributes) ? V_Helper_Html::attributes(
        $attributes) : '') . '>' . // Title empty? Use the parsed URL
(($title === null) ? $site_url : $title) . '</a>';
     ...
     * @return string
     */
    public static function file_anchor($file, $title = null, 
    $attributes = null, $protocol = null)
    {
        return // Base URL + URI = full URL
'<a href="' . V_Helper_Html::specialurlencode(
        url::base(false, $protocol) . $file, 
        false) . '"' . // Attributes empty? Use an empty string
(is_array($attributes) ? V_Helper_Html::attributes(
        $attributes) : '') . '>' . // Title empty? Use the filename part of the URI
(($title === null) ? end(
        explode('/', $file)) : $title) . '</a>';
    }
     ...
     * @return string
     */
    public static function panchor($protocol, $uri, 
    $title = null, $attributes = false)
    {
        return V_Helper_Html::anchor($uri, 
        $title, $attributes, $protocol);
    }
    ...
    $attributes = null)
    {
        ...
        $safe = V_Helper_Html::email(
        $email);
        // Title defaults to the encoded email address
        empty(
        $title) and $title = $safe;
        // Parse attributes
        empty(
        $attributes) or $attributes = V_Helper_Html::attributes(
        $attributes);
    ...
    public static function breadcrumb($segments = null)
    {
        ...
        while($segment = array_pop($segments))
        {
            ...
            implode(
            '/', $segments) . '/' . $segment,  // Title for the current segment
            ucwords(
            inflector::humanize(
            $segment)));
        }
    ...
    public static function meta($tag, $value = null)
    {
        ...
        $attr = in_array(
        strtolower($tag), 
        Kohana::config('http.meta_equiv')) ? 'http-equiv' : 'name';
    ...
    $index = false)
    {
        return V_Helper_Html::link($style, 
        'stylesheet', 'text/css', $media, $index);
    }
     ...
     * @return string
     */
    public static function link($href, $rel, $type, 
    $media = false, $index = false)
    {
        $compiled = '';
        if(is_array($href))
        {
        ...
        else
        {
            ...
            $compiled = '<link' . V_Helper_Html::attributes(
            $attr) . ' />';
        }
        return $compiled . "\n";
    }
     ...
     * @return string
     */
    public static function image($src = null, $alt = null, 
    $index = false)
    {
        // Create attribute list
        $attributes = is_array(
        $src) ? $src : array('src' => $src);
        if(is_array($alt))
        {
            $attributes += $alt;
        }
        ...
        if(strpos($attributes['src'], '://') === false)
        {
            ...
            $attributes['src'] = url::base(
            $index) . $attributes['src'];
        }
        return '<img' . V_Helper_Html::attributes(
        $attributes) . ' />';
    }
    ...
    public static function attributes($attrs)
    {
        ...
        {
            $compiled .= ' ' . $key . '="' . V_Helper_Html::specialchars(
            $val) . '"';
        }
        return $compiled;
    }
