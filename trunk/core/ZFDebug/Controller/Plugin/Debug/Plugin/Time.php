...
class ZFDebug_Controller_Plugin_Debug_Plugin_Time extends Zend_Controller_Plugin_Abstract implements ZFDebug_Controller_Plugin_Debug_Plugin_Interface
{
    ...
    public function getPanel()
    {
        ...
        2) . ' ms<br />';
        if(isset($this->_timer['user']) && count(
        $this->_timer['user']))
        {
        ...
        $timerNamespace = new Zend_Session_Namespace(
        'ZFDebug_Time', false);
    ...
    public function mark($name)
    {
        if(isset($this->_timer['user'][$name]))
            $this->_timer['user'][$name] = (microtime(
            true) - $_SERVER['REQUEST_TIME']) * 1000 - $this->_timer['user'][$name];
        else
            $this->_timer['user'][$name] = (microtime(
            true) - $_SERVER['REQUEST_TIME']) * 1000;
    }
    ...
    Zend_Controller_Request_Abstract $request)
    {
        ...
        $this->_timer['preDispatch'] = (microtime(
        true) - $_SERVER['REQUEST_TIME']) * 1000;
    }
    ...
    Zend_Controller_Request_Abstract $request)
    {
        $this->_timer['postDispatch'] = (microtime(
        true) - $_SERVER['REQUEST_TIME']) * 1000;
    }
    ...
    protected function _calcAvg(array $array, $precision = 2)
    {
        ...
        $cuantos = count($array);
        return round(
        array_sum($array) / $cuantos, $precision);
    }
}
