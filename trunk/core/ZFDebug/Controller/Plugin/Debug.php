...
class ZFDebug_Controller_Plugin_Debug extends Zend_Controller_Plugin_Abstract
{
    ...
    public function __construct($options = null)
    {
        ...
        $version->setPanel(
        $this->_getVersionPanel())->setTab(
        $this->_getVersionTab())->setIdentifier(
        'copyright');
    ...
    public function dispatchLoopShutdown()
    {
        ...
        foreach($this->_plugins as $plugin)
        {
            ...
            $html .= '<img src="' . $this->_icon(
            $plugin->getIdentifier()) . '"  width="16" height="16" style="vertical-align:middle" alt="' . $plugin->getIdentifier() . '" title="' . $plugin->getIdentifier() . '" /> ';
            $html .= $tab . '</span>';
        }
    ...
    {
        $collapsed = isset(
        $_COOKIE['ZFDebugCollapsed']) ? $_COOKIE['ZFDebugCollapsed'] : 0;
        return ('
