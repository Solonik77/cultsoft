        ...
        array $config = array())
    {
        ...
        switch ($config['transport']) {
            case 'smtp':
                ...
                if (! empty($config['auth']) and ! in_array($config['auth'],
                            array('crammd5' , 'login' , 'plain'))) {
                    throw new App_Exception('Wrong auth parameter for smtp connection.');
                }
                App_Mail::$transport = new Zend_Mail_Transport_Smtp($config['host'],
                    $config);
                break;
            default:
