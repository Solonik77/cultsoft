...
class App_Controller_Plugin_View extends Zend_Controller_Plugin_Abstract {
    ...
    */
    ...
    private function _initTemplatePath()
    {
        ...
        $this->_templatePath = STATIC_PATH . 'templates/' . App::config()->project->template . '/';
    ...
    private function _initView()
    {
        ...
        $this->_view->addHelperPath(CORE_PATH . 'App/View/Helper/', 'App_View_Helper');
    }
    ...
    private function _initViewRenderer()
    {
        ...
        $this->_viewRenderer->setView($this->_view)->setViewBasePathSpec($this->_templatePath)->setViewScriptPathSpec(
            (($this->_isBackofficeController !== true) ? 'html/:module/:controller/:action.:suffix' : 'html/:controller/:action.:suffix'))->setViewScriptPathNoControllerSpec(
    ...
    private function _initZendLayout(Zend_Controller_Request_Abstract $request)
    {
        $layout = Zend_Layout::startMvc(
            array('layoutPath' => $this->_templatePath . 'layouts/' , 'layout' => 'default' , 'mvcSuccessfulActionOnly' => ('development' === APPLICATION_ENV)));
        if (! $this->_isBackofficeController) {
    ...
    private function _loadDefaultTemplateResources()
    {
        ...
        $this->_view->getHelper('HeadMeta')->appendHttpEquiv('Content-Type',
        ...
        $this->_view->getHelper('HeadScript')->appendFile(
            App::baseUri() . 'static/system/clientscripts/jquery.js');
        $this->_view->getHelper('HeadScript')->appendFile(
            App::baseUri() . 'static/system/clientscripts/swfobject.js');
        $this->_view->getHelper('HeadScript')->appendFile(
            App::baseUri() . 'static/system/clientscripts/jquery/pngfix.js');
    ...
    private function _loadSiteTemplateResources()
    {
        ...
        $this->_view->getHelper('HeadLink')->appendStylesheet(App::baseUri() . 'static/templates/' . App::config()->project->template . '/css/styles.css');
    }
    ...
    private function _loadAdminTemplateResources()
    {
        ...
        $this->_view->getHelper('HeadMeta')->appendHttpEquiv('Designer',
            'ne-design (www.ragard-jp.com) KuroAdmin Theme');
        $this->_view->getHelper('HeadLink')->appendStylesheet(
            App::baseUri() . 'static/system/admin/css/elements.css');
        $this->_view->getHelper('HeadLink')->appendStylesheet(
            App::baseUri() . 'static/system/css/smoothness/jquery.ui.css');
        $this->_view->getHelper('HeadScript')->appendFile(
            App::baseUri() . 'static/system/clientscripts/jquery/ui.js');
    }

    private function _declareDefaultVars()
    {
        $requestLang = App::Front()->getParam('requestLang');
        $this->_view->getHelper('DeclareVars')->declareVars(array('uploadedIMG' => App::baseUri() . 'static/upload/images/' ,
                'requestLang' => $requestLang , 'projectTitle' => App::config()->project->title->$requestLang , 'baseUrl' => App::baseUri() ,
                'tplJS' => App::baseUri() . ((! $this->_isBackofficeController) ? 'static/templates/' . App::config()->project->template . '/clientscripts/' : 'static/system/admin/clientscripts/') ,
                'tplCSS' => App::baseUri() . ((! $this->_isBackofficeController) ? 'static/templates/' . App::config()->project->template . '/css/' : 'static/system/admin/css/') ,
                'tplIMG' => App::baseUri() . ((! $this->_isBackofficeController) ? 'static/templates/' . App::config()->project->template . '/images/' : 'static/system/admin/images/')));
    }
}
