...
class App_Controller_Plugin_View extends Zend_Controller_Plugin_Abstract
{
    ...
    Zend_Controller_Request_Abstract $request)
    {
        try
        {
            ...
            if(Zend_Registry::get(
            'BACKOFFICE_CONTROLLER') == true and Zend_Registry::get(
            'member_access') == 'ALLOWED')
            {
            ...
            $view->addHelperPath(
            CORE_PATH . 'App/View/Helper/', 
            'App_View_Helper');
            $viewRenderer = Zend_Controller_Action_HelperBroker::getStaticHelper(
            'viewRenderer');
            $viewRenderer->setView(
            $view)->setViewBasePathSpec(
            $template_path)->setViewScriptPathSpec(
            (($backoffice_controller !== true) ? 'html/:module/:controller/:action.:suffix' : 'html/:controller/:action.:suffix'))->setViewScriptPathNoControllerSpec(
            (($backoffice_controller !== true) ? 'html/:module/:action.:suffix' : 'html/:action.:suffix'))->setViewSuffix(
            'phtml');
            $layout = Zend_Layout::startMvc(
            array('layoutPath' => $template_path . 'layouts/' , 'layout' => 'default' , 'mvcSuccessfulActionOnly' => ('development' === APPLICATION_ENV)));
            if(! $backoffice_controller)
            {
                if($request->getModuleName() == 'default' and $request->getControllerName() == 'index' and $request->getActionName() == 'index' and file_exists(
                $template_path . 'layouts/homepage.phtml'))
                {
                    $layout->setLayout(
                    'homepage');
                }
                    ...
                    if(file_exists(
                    $template_path . 'layouts/' . $request->getModuleName() . '.phtml'))
                    {
                        $layout->setLayout(
