...
class App_Exception extends Zend_Exception
{
    ...
    public function __toString()
    {
        ...
        {
            if(method_exists(
            $this, 'sendHeaders') and ! headers_sent())
            {
            ...
            // Log the error
            App::log(
            'Uncaught ' . $type . ' ' . $this->message . ' in file ' . $this->debug_path(
            ...
            'headStyle')->headStyle(
            file_get_contents(
            STATIC_PATH . 'system/css/error' . ((APPLICATION_ENV != 'development') ? '_disabled' : '') . '.css'));
            $layout->setLayoutPath(
            STATIC_PATH . 'system/')->setLayout(
            'system_error');
            $message = $this->message;
            $file = $this->debug_path(
            $this->file);
            $line = $this->line;
            if(! empty(
            $this->file))
            {
                ...
                $file_source))
                {
                    if($file_line >= $start_line)
                    {
                        ...
                        {
                            $read_line = v_helper_html::specialchars(
                            $read_line);
                        }
                        $source .= $read_line;
                    }
        ...
        catch(Exception $e)
        {
            ...
            {
                exit(
                'Exception thrown inside ' . __CLASS__ . ': ' . $e->getMessage());
            }
            else
            {
                exit(
    ...
    public function read_trace(array $trace_array)
    {
        ...
        foreach($trace_array as $trace)
        {
            ...
            $trace['function']))
            {
                ...
                $inline = in_array(
                $trace['function'], 
                array('require' , 'require_once' , 'include' , 'include_once' , 'echo' , 'print'));
                $line = array();
                if(isset(
                $trace['class']))
                {
                ...
                $trace['args']))
                {
                    foreach($trace['args'] as $arg)
                    {
                        ...
                        $args[] = '<code>' . V_Helper_Text::limit_chars(
                        v_helper_html::specialchars(
                        self::debug_var(
                        $arg)), 
                        50, 
                        '...') . '</code>';
    ...
    public function debug_path($file)
    {
        ...
        {
            $file = 'CORE_PATH/' . substr(
            $file, 
            strlen(CORE_PATH));
        }
        elseif(strpos($file, DOC_ROOT) === 0)
        {
            $file = 'DOC_ROOT/' . substr(
            $file, 
            strlen(DOC_ROOT));
        }
        return $file;
