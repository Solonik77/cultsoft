    ...
    */
    public function __toString()
    {
        try {
            ...
            // Log the error
            App::log('Uncaught ' . $type . ' ' . $this->message . ' in file ' . $this->debug_path($this->file) . ' on line ' . $this->line,
                Zend_Log::ERR);
            $layout = new Zend_Layout();
            $view = $layout->getView();
            $view->getHelper('headTitle')->headTitle("System Error");
            $view->getHelper('headStyle')->headStyle(
                file_get_contents(
                    STATIC_PATH . 'system/css/error' . ((APPLICATION_ENV != 'development') ? '_disabled' : '') . '.css'));
            $layout->setLayoutPath(
                STATIC_PATH . 'system/')->setLayout('system_error');
    ...
    public function read_trace(array $trace_array)
    {
        ...
        foreach($trace_array as $trace) {
            ...
            if (isset($trace['function'])) {
                ...
                $inline = in_array($trace['function'],
    ...
    */
    public function debug_path($file)
    {
        if (strpos($file, APPLICATION_PATH) === 0) {
            $file = 'APPLICATION_PATH/' . substr($file,
                strlen(
                    APPLICATION_PATH));
        } elseif (strpos($file, CORE_PATH) === 0) {
            $file = 'CORE_PATH/' . substr($file,
                strlen(CORE_PATH));
        } elseif (strpos($file, DOC_ROOT) === 0) {
            $file = 'DOC_ROOT/' . substr($file,
                strlen(DOC_ROOT));
        }
        return $file;
    }
    ...
    public static function debug_var($var, $recursion = false)
    {
        ...
        switch (gettype($var)) {
            case 'object':
                ...
                if ($recursion === true and in_array($hash,
                        $objects)) {
                    $out .= '*RECURSION*';
                } else {
                    ...
                    foreach($object->getProperties() as $property) {
                        ...
                        if ($property->isPublic()) {
                            $out .= self::debug_var($property->getValue($var),
                                true);
