...
if ($this->getRequest()->isPost()) {
 ...
 if (! $form->isValid($_POST)) {
  ...
  if (! $result->isValid()) {$form->addDecorator('Description', array('escape' => true , 'placement' => 'prepend'));$form->setDescription('Wrong email or password');$this->view->form = $form;
  } else {App_Member::getAuth()->getStorage()->write($authAdapter->getResultRowObject(  array('id' , 'email')));if (isset($formData['remember_me'])) { Zend_Session::rememberMe(3600 * 24 * 14);}$this->_redirect('admin');
  }
 }
} else {
 $this->view->form = $form;
