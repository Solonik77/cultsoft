...
class yiiDebugClass {
    ...
    public static function render($items)
    {
        ...
        $result = '<table><tr><thead><th>Time</th><th>Level</th><th>Category</th><th width="100%">Message</th></td></tr></thead>' . $result . '</table>';
...
class yiiDebugIncFiles extends yiiDebugClass {
    public static function getInfo($data)
    {
        parent::getInfo($data);
        $result = array();
        $result['title'] = 'Included files: ' . count(get_included_files());
        $result['panelTitle'] = 'List of included files.';
        $result['content'] = self::compile(get_included_files());
...
class yiiDebugConfig extends yiiDebugClass {
    ...
    public static function getHeadInfo()
    {
        ...
        $config = array('debug' => (DEFINED('YII_DEBUG') && YII_DEBUG != false) ? 'on' : 'off',
    ...
    public function yiiAppAsArray()
    {
        $result = Yii::app();
        return Yii::app();
    }

    protected function formatArrayAsHtml($id, $values, $highlight = false)
    {
...
class XWebDebugRouter extends CLogRoute {
    ...
    public function processLogs($logs)
    {
        ...
        $items[] = yiiDebugIncFiles::getInfo($logs);
