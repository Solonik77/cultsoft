...
{
    if (in_array($extension, $types)) return false;
    else return true;
}
...
function alert(&$notify)
{
    ...
    for($i = 0;$i < $alert_num;$i++) {
        ...
        ?><div class="alert<?php echo $notify['type'][$i];
        ?>"><?php echo $notify['message'][$i];
        ?></div><br /><?php
    }
}
...
function form_open($name, $class, $url, $parameters)
{
    ...
    ?><form name="<?php echo $name;
    ?>" class="<?php echo $class;
    ?>" method="post" action="<?php echo $url . $parameters;
    ?>">
<?php
}
...
function form_submit_button($name, $label, $class)
{
    ...
    ?><button <?php if ($class) {
        ?>class="<?php echo $class;
        ?>"<?php }
    ?>type="submit" name="<?php echo $name;
    ?>"><?php echo $label;
    ?></button>
</form>
<?php
...
function clean_filename($filename)
{
    ...
    $filename = preg_replace("/\s+/ms", "-", $filename);
    $filename = strtr($filename, $NpjCaps, $NpjSmall);
    $filename = strtr($filename, $NpjLettersFrom, $NpjLettersTo);
    $filename = strtr($filename, $NpjBiLetters);

    $filename = preg_replace("/[^a-z0-9\_\-.]+/mi", "", $filename);
    ...
    if (strlen($filename) > 50) {
        ...
        if (($temp_max = strrpos($filename, '-'))) {
            $filename = substr($filename, 0, $temp_max);
        }
    }
...
function dirtree(&$alldirs, $types = '*.*', $root = '', $tree = '', $branch = '', $level = 0)
{
    ...
    while (($dirname = readdir($dh)) !== false) {
        if ($dirname != '.' && $dirname != '..' && $dirname != '.svn' && is_dir($root . $tree . $branch . $dirname) && $dirname != '_thumbs') {
            $filenum = 0;
            foreach($filetypes as $filetype) {
