...
class html {
    ...
    public static function videoplayer($params)
    {
    
     $html =  "<div id='" . $params['div'] . "'> </div>
    <script type='text/javascript'>
        var so = new SWFObject('/assets/flash/player.swf','ply','" . $params['width'] . "','" . $params['height'] . "','9','#ffffff');
        so.addParam('allowfullscreen','true');
        so.addParam('allowscriptaccess','always');
        so.addParam('wmode','opaque');
        so.addVariable('dock','false');
        so.addVariable('icons','false');
        so.addVariable('file','" . Yii::app()->request->getBaseUrl(true) . "/static/" . $params['file'] . "');
        so.addVariable('image','" . Yii::app()->request->getBaseUrl(true) . "/static/" . $params['image'] . "');
        so.write('" . $params['div'] . "');
    </script>";
    return $html;
    }
